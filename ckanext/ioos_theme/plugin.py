...
def gcmd_to_ul(gcmd_dict, elem=etree.Element('ul', {'class': 'tag-list'}),
               prev_results=None):
    ...
    for sub_key, sub_dict in gcmd_dict.items():
        ...
        gcmd_link = etree.SubElement(gcmd_list, 'a',
                                        #{'href': 'https://data.ioos.us/search?{}'.format(exploded_kw)})
                                        {'href': 'http://localhost:5000/dataset?q=gcmd_keywords_full:"{}"'.format(exploded_kw)})
        gcmd_link.text = sub_key
        if sub_dict:
            new_ul = etree.SubElement(elem, 'ul', {'class': 'tag-list'})
